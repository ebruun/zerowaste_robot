...
from communication.client_container import ClientContainer
from communication.messages.messagetypes import MSG_CURRENT_POSE_CARTESIAN, MSG_CURRENT_POSE_JOINT, MSG_CURRENT_POSE_CARTESIAN_BASE, MSG_COMMAND, MSG_STOP
from communication.messages.messagetypes import CMD_PICK_ROD, CMD_MILL_ROD_END, CMD_MILL_ROD_START, CMD_REGRIP_PLACE, CMD_REGRIP_PICK, CMD_SAFE_POS, CMD_OPEN_GRIPPER, CMD_CLOSE_GRIPPER, CMD_OPEN_CLAMP
from communication.messages.messagetypes import CMD_GO_TO_TASKTARGET, CMD_GO_TO_TASKTARGET_JOINTS, CMD_GO_TO_JOINTTARGET_ABS, CMD_GO_TO_JOINTTARGET_REL, CMD_PICK_BRICK, CMD_PLACE_BRICK, CMD_PICK_BRICK_FROM_POSE
from communication.messages.messagetypes import CMD_STU_PICK, CMD_STU_PLACE_1, CMD_STU_PLACE_2
from communication.messages.messagetypes import CMD_MAS_PICK, CMD_MAS_PLACE, CMD_MAS_PICK_MAGAZINE, CMD_MAS_PLACE_MAGAZINE, CMD_LWS_DYNAMIC_PICKUP, CMD_RAPID_STOP, CMD_PULSEDO
from communication.messages.messagetypes import CMD_SENDMOVELRELTOOL, CMD_SENDMOVELRELTCP, CMD_COORDINATED_GANTRY_MOVE, CMD_SET_SPEED_INPUT
...
class ABBCommunication(ClientContainer):
    ...
    def __init__(self, identifier, host='127.0.0.1', port_snd=30003, port_rcv=30004, ghenv = None):
        ...
        self.int_speed = 0 # speed: 0 = slow, 1 = mid, 2 = fast, 3 = superslow....for some reason "fast not working"
        self.float_duration = 0 # duration is not used, use velocity instead
        self.int_zonedata = 10 # zonedata: in mm
        self.int_tool = 0 
        self.int_wobj = 0 # 0 = wobj0, 1 = wobj0, 2 = rotrobwob_r1_cg_04 (what edvard is using), 
        self.int_rob_num = 0 # NOT USED: right now it does nothing
        self.float_arbitrary = 0 # robot number to access (does what above should do)
    ...
    def get_ext_axes_eb(self, ext_axes_in):
        if type(ext_axes_in) == list or type(ext_axes_in) == tuple:
            if len(ext_axes_in) == 1:
                ext_axes = [ext_axes_in[0], 0, 0]
            elif len(ext_axes_in) == 2:
    ...
    def send_pose_cartesian(self, input, ext_axes_in = DEFAULT_AXES, int_arr=None):
        ...
        pose = self.get_pose_eb(input)
        ext_axes = self.get_ext_axes_eb(ext_axes_in)
    ...
    def send_pose_joint_home(self, int_arr=None):
        ...
        print(self.jointtarget_home_zero)
    ...
    def set_speed_to_num(self, speed_num):
        self.int_speed = speed_num

        if self.int_speed == 0:
            self.int_zonedata = 10
        elif self.int_speed == 1:
            self.int_zonedata = 20
        elif self.int_speed == 2:
            self.int_zonedata = 100
        elif self.int_speed == 3:
            self.int_zonedata == 10
